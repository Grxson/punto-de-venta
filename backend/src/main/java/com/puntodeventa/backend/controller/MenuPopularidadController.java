 ...
 * arriba hacia abajo, ordenados por popularidad (ventas, frecuencia, ingresos,
 * etc.)
 */
@RestController
@RequestMapping(value = "/api/v1/menu", produces = "application/json")
...
public class MenuPopularidadController {
         ...
         */
        @GetMapping("/ordenado")
        @Operation(summary = "Obtener menú ordenado por popularidad", description = "Retorna el menú con productos distribuidos en grilla, ordenados por popularidad")
        public ResponseEntity<MenuGrillaDTO> obtenerMenuOrdenado(
                        @Parameter(description = "Columnas en la grilla", example = "3") @RequestParam(defaultValue = "3") int columnasGrid,
                        @Parameter(description = "Días para análisis de popularidad", example = "7") @RequestParam(defaultValue = "7") int diasAnalizar) {
         ...
         * Cada categoría tiene su propia grilla con productos ordenados por
         * popularidad.
         *
        ...
        @Operation(summary = "Obtener menú ordenado por categoría y popularidad", description = "Retorna el menú agrupado por categorías, cada una con su propia grilla de productos")
        public ResponseEntity<MenuGrillaDTO> obtenerMenuPorCategoria(
                        @Parameter(description = "Columnas por categoría", example = "3") @RequestParam(defaultValue = "3") int columnasGrid,
                        @Parameter(description = "Días para análisis", example = "7") @RequestParam(defaultValue = "7") int diasAnalizar) {
         ...
         */
        @GetMapping("/top")
        @Operation(summary = "Obtener top N productos por popularidad", description = "Retorna los productos más populares ordenados descendente")
        public ResponseEntity<List<ProductoPopularidadDTO>> obtenerTopProductos(
                        @Parameter(description = "Número de productos", example = "10") @RequestParam(defaultValue = "10") int limite,
                        @Parameter(description = "Días para análisis", example = "7") @RequestParam(defaultValue = "7") int diasAnalizar) {
         ...
         */
        @GetMapping("/grilla")
        @Operation(summary = "Obtener distribución en grilla", description = "Retorna la estructura de grilla con posiciones (fila, columna) de cada producto")
        public ResponseEntity<MenuGrillaDTO> obtenerDistribucionGrilla(
                        @Parameter(description = "Columnas en grilla", example = "3") @RequestParam(defaultValue = "3") int columnasGrid,
                        @Parameter(description = "Días para análisis", example = "7") @RequestParam(defaultValue = "7") int diasAnalizar) {
        ...
        @Operation(summary = "Obtener estadísticas de popularidad", description = "Retorna datos detallados de popularidad para cada producto")
