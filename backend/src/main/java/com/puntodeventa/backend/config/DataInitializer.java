        ...
        return args -> {
            try {
                ...
                if (metodoPagoService != null) {
                    try {
                        ...
                        if (metodosExistentes.isEmpty()) {
                            ...
                            metodoPagoService
                                    .crear(new MetodoPagoDTO(null, "Efectivo", false, true, "Pago en efectivo"));
                            metodoPagoService.crear(new MetodoPagoDTO(null, "Tarjeta", false, true,
                                    "Pago con tarjeta de débito o crédito"));
                            metodoPagoService.crear(
                                    new MetodoPagoDTO(null, "Transferencia", true, true, "Transferencia bancaria"));

                            System.out.println(">>> ✅ Métodos de pago cargados (Efectivo, Tarjeta, Transferencia)");
                        } else {
                            System.out.println(
                                    ">>> Métodos de pago ya existen (" + metodosExistentes.size() + " métodos)");
                        }
                    } catch (Exception e) {
                        System.err.println(
                                ">>> ⚠️  No se pudieron cargar métodos de pago usando servicio: " + e.getMessage());
                        ...
                        try {
                            ...
                            Long metodoPagoCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM metodos_pago",
                                    Long.class);
                            ...
                            System.err.println(">>> ❌ Error en segundo intento de cargar métodos de pago: "
                ...
                try {
                    ...
                    Long categoriaCount = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM categorias_productos",
                            Long.class);
                    if (categoriaCount == null || categoriaCount == 0) {
                        ...
                        crearProductoConVariantes(jdbcTemplate,
                                "Verde", "Jugo verde de verduras y frutas (apio, perejil, nopal, espinaca, piña)", 1L,
                                40.00,
                        ...
                        jdbcTemplate.update(
                                "INSERT INTO productos (nombre, descripcion, categoria_id, precio, activo, disponible_en_menu) "
                                        ...
                                        "VALUES (?, ?, ?, ?, ?, ?)",
                                "Chilaquiles",
                                "Orden de chilaquiles rojos acompañados de frijoles, huevo al gusto y bolillo", 3,
                                65.00, true, true);
                        jdbcTemplate.update(
                                "INSERT INTO productos (nombre, descripcion, categoria_id, precio, activo, disponible_en_menu) "
                                        +
                                        "VALUES (?, ?, ?, ?, ?, ?)",
                                "Huevos al gusto", "Orden de Huevos al gusto acompañados de frijoles y bolillo", 3,
                                50.00, true, true);
                        ...
                        jdbcTemplate.update(
                                "INSERT INTO productos (nombre, descripcion, categoria_id, precio, activo, disponible_en_menu) "
                                        +
                                        "VALUES (?, ?, ?, ?, ?, ?)",
                                "Molletes Dulces", "Molletes dulces con mantequilla, azúcar y canela", 3, 30.00, true,
                                true);
                        jdbcTemplate.update(
                                "INSERT INTO productos (nombre, descripcion, categoria_id, precio, activo, disponible_en_menu) "
                                        +
                                        "VALUES (?, ?, ?, ?, ?, ?)",
                                "Molletes con Untado", "Molletes con untado de mermelada, nutella, lechera o miel", 3,
                                35.00, true, true);
                        jdbcTemplate.update(
                                "INSERT INTO productos (nombre, descripcion, categoria_id, precio, activo, disponible_en_menu) "
                                        +
                                        "VALUES (?, ?, ?, ?, ?, ?)",
                                "Molletes Salados", "Molletes salados con frijoles con queso o salsa mexicana", 3,
                                40.00, true, true);
                        ...
                        crearProductoSimple(jdbcTemplate, "Lonche de Jamón y Panela", "Lonche de jamón y panela", 3L,
                                55.00);
                        ...
                        crearProductoSimple(jdbcTemplate, "Sandwich de Jamón y Panela", "Sandwich de jamón y panela",
                                3L, 45.00);
                        crearProductoSimple(jdbcTemplate, "Sandwich de Chilaquiles", "Sandwich de chilaquiles", 3L,
                                35.00);
                        ...
                        Long productosBase = jdbcTemplate.queryForObject(
                                "SELECT COUNT(*) FROM productos WHERE producto_base_id IS NULL", Long.class);
                        Long variantes = jdbcTemplate.queryForObject(
                                "SELECT COUNT(*) FROM productos WHERE producto_base_id IS NOT NULL", Long.class);
