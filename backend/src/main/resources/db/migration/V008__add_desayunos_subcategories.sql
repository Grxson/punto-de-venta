...
SELECT id, 'DULCES', 'Molletes, Waffles, Mini Hot-Cakes', 1, true
FROM categorias_productos WHERE nombre = 'Desayunos'
ON CONFLICT (categoria_id, nombre) DO NOTHING;

INSERT INTO categoria_subcategorias (categoria_id, nombre, descripcion, orden, activa)
SELECT id, 'LONCHES', 'Lonches, Sándwiches de Lonche', 2, true
FROM categorias_productos WHERE nombre = 'Desayunos'
ON CONFLICT (categoria_id, nombre) DO NOTHING;

INSERT INTO categoria_subcategorias (categoria_id, nombre, descripcion, orden, activa)
SELECT id, 'SANDWICHES', 'Sándwiches', 3, true
FROM categorias_productos WHERE nombre = 'Desayunos'
ON CONFLICT (categoria_id, nombre) DO NOTHING;

INSERT INTO categoria_subcategorias (categoria_id, nombre, descripcion, orden, activa)
SELECT id, 'OTROS', 'Otros productos de desayuno', 4, true
FROM categorias_productos WHERE nombre = 'Desayunos'
ON CONFLICT (categoria_id, nombre) DO NOTHING;
