...
## Problema
La migración de Hibernate intenta hacer la columna `activo` de la tabla `metodos_pago` como `NOT NULL`, pero:
1. Existen registros con valores `NULL` en esa columna
2. La columna es de tipo `INTEGER` (0/1), no BOOLEAN
...
-- 2. Añadir default 1 a la columna
